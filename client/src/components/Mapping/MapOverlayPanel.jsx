...
class MapOverlayPanel extends Component {
  ...
  drawScenarioLayer = () => {
    ...
    this.props.setActiveDevType({
      devTypeName: "DRAW_SCENARIO_BASE_LAYER",
      color: ""
    });
  };
  resestScenarioLayer = () => {
  ...
  render() {
    ...
    return (
      <span className={classes.overlayContainer}>
        ...
        >
          {this.props.mapOverlayPanel === "painting" ? (
            ...
            <span>
              ...
              {activeDevType.devTypeName === "DRAW_SCENARIO_BASE_LAYER" ? (
                <MapDrawHelper {...this.props} />
              ) : (
                ""
              )}
        ...
        {this.props.mapOverlayPanel === false ? (
          <span />
        ) : (
          <Paper
            variant="raised"
          ...
          >
            ...
            <Button
              ...
              style={{
                ...
                background: "#eeeeee",
                outline:
                  null === activeDevType.devTypeName ? "2px solid #ccc" : ""
              }}
            ...
            {null === activeDevType.devTypeName ? (
              <MapDrawHelper {...this.props} />
            ) : (
              ""
            )}
            {workbook_devtypes.map((item, idx) => {
              let localDevTypeName =
                item.devTypeName.length <= 1 ? "<empty>" : item.devTypeName;
              return (
                <div key={idx}>
                  <Button
                    variant="raised"
                    ...
                    style={{
                      ...
                      outline:
                        localDevTypeName === activeDevType.devTypeName
                          ? "2px solid #ccc"
                          : ""
                    }}
                    onClick={() =>
                      this.paintDevelopmentType(
                        localDevTypeName,
                        sidePalette[idx % sidePalette.length].fill
                      )
                    }
                  >
                    {" "}
